...
def search_results():
    ...
    with open("kb/images.json", "r" , encoding="utf-8") as images:
...
def write_to_csv(data:list, file_name:str):
    ...
    with open("kb/{}.csv".format(file_name) , "w" , encoding="utf-8") as op_csv:
            ...
            for row in data:
                writer.writerow({"openshift_images":row["openshift_images"],'container_name': row["container_name"], 'OS': row["OS"] \
                    , "lang":row["lang"],  "lib": row["lib"] , "app": row["app"] \
                        ,"app_server": row["app_server"] , "plugin":row["plugin"] , \
                            "runlib": row["runlib"] , "runtime": row["runtime"] , \
                            "Opeshift_Correspondent_Image_Url": row["Opeshift_Correspondent_Image_Url"] ,\
...
def operator_images():
    ...
    for op in op_images:
        ...
        for operator in op[op_name]:
            ...
            op_data["Operator_Correspondant_Image_Url"] = operator["container_images"]
        ...
        if len(exact_images) != 0: 
            ...
            for ky, val in exact_images.items():
                ...
                if ky not in ["type","entity_id"]:
                    ...
                    img_data["Notes"] = ""
                    img_data["CertOfImageAndPublisher"] = val[0]["Official image"]
        ...
        if len(exact_images) != 0: 
            for ky, val in exact_images.items():
                if ky not in ["type","entity_id"]:
                    ...
                    img_data["container_name"] = val[0]["name"]
                    img_data["Opeshift_Correspondent_Image_Url"] = val[0]["url"]
