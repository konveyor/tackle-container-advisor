...
config_data = os.path.join("src/config.ini")
...
with open("kb/verified_publisher_names.json", "r", encoding="utf-8") as verified_file:
...
class Operators():
    ...
    def community_op_images_repos(self, operator_name: str) -> list:
        ...
        try:
            ...
            for imgs in image_urls:
                container_images.append(imgs['image'])
    ...
    def community_operators(self, entity: str):
        ...
        """
        sumary_line: Search community  operators from https://artifacthub.io/packages/olm/community-operators/
        
        Keyword arguments:
        entity (str) -- An entity name from from the database
        Return: list of all matching opertors 
        """
...
class Quay:
    ...
    def popular_images(self, quay_images:dict, top:int):
        ...
        Args:
            quay_images (dict): List of Quay imaes 
            top (int): Threshold = 5 
...
class DockerHubSearch():
    ...
    def verified_official_images(self, images:list):
        """
        Retrieve all verified a official images 
    ...
    def recommend_exact_image(self,entity_name:str , verified_and_official_images:list):
        """
        sumary_line: Searches for  exact dockerhub images
    ...
    def recommend_top_images ( self,entity_name:str , verified_and_official_images:list)->list:
        ...
        """
        _summary_: Recommend top relevant images from dockerhub 
    ...
    def recommended_images(self, image_inst:list, entity_name:str):
        """
        sumary_line: Given an entity_name and a list of relevant images from DockerHub, determine the exact match and the top relevant matches.
    ...
    def search_dockerhub_images(self, entity: str) -> tuple:
        """
        sumary_line:  Search container iages from dockerhub

        Args:
            entity (str): An entity(query) to search
        ...
        try:
            images = self.client.images.search(term=entity, limit=50)
        except:
            print("ImageNotFound error occured, check your entry and try again")
        if images == []:
...
def save_to_kb(results: list) -> None:
    ...
    with open("kb/images.json", "w" , encoding="utf-8") as images_file:
